...
MorphApplicator::MorphApplicator(BSTriShape * _geometry, UInt8 * srcBlock, UInt8 * dstBlock, std::function<void(std::vector<Morpher::Vector3> &)> morph) : geometry(_geometry), morphFunc(morph)
{
	...
	auto* triangles = (geomData->triangleData != nullptr) ? reinterpret_cast<Morpher::Triangle*>(geomData->triangleData->triangles) : nullptr;
...
void MorphApplicator::RecalcNormals(UInt32 numTriangles, Morpher::Triangle* triangles, bool smooth, float smoothThresh)
{
	const auto numVertices = rawVertices.size();
	...
	// Smooth normals
	if (smooth) {
		const auto matches = kd_matcher::find_matches(verts);
		for (std::size_t i = 0; i < matches.size(); ++i)
		{
			const auto& a = matches[i].first;
			const auto& b = matches[i].second;
...
void MorphApplicator::CalcTangentSpace(UInt32 numTriangles, Morpher::Triangle* triangles)
{
	...
	for (UInt32 i = 0; i < numTriangles; ++i)
	{
		...
		const Morpher::Vector3& v1 = rawVertices[i1];
		const Morpher::Vector3& v2 = rawVertices[i2];
		const Morpher::Vector3& v3 = rawVertices[i3];

		const Morpher::Vector2& w1 = rawUV[i1];
		const Morpher::Vector2& w2 = rawUV[i2];
		const Morpher::Vector2& w3 = rawUV[i3];
